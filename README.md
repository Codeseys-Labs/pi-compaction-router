...
A model row also carries what this session has already *observed* about that exact target: `last call ok`, or `LAST CALL FAILED (<class>)`. That is read from calls the router already made — the same record `/compaction-router` prints — so it costs no extra call and no provider is asked anything. A target nothing is recorded against says nothing, because silence is an absence rather than a verdict.

**What the write confirms, and what it cannot.** A pick is validated by resolving it in the catalogue. That is not an invocability check, and the two differ in practice: several listed Bedrock ids refuse every on-demand call with `Invocation of model ID <id> with on-demand throughput isn't supported` and are reachable only through their `us.`/`global.`-prefixed inference-profile entries — which are *separate catalogue entries* that resolve identically. So the confirmation says which check passed, states that invocability is discovered on the first real compaction (which fails open to Pi's active model and says so), and names the prefixed forms as the remedy. If a target you pick has already failed a call this session, the write is warned about rather than refused: a recorded failure can be stale, and refusing on a memory would make a transient failure unpickable until restart.
...
- **A fallback nobody was present for is mentioned once more, on the next compaction that routes.** The banner above is retracted when the next compaction begins, which is correct — it describes a compaction that one supersedes — but it means an operator away from the terminal during a *transient* fallback would never learn a target had failed. So one line is carried forward, exactly one compaction, and only when that compaction did **not** itself fall back: a persistent fault raises its own live banner and does not need a second notice. It has its own widget key, is retracted on the same schedule as the others, and does not repeat. A degradation is also visible without it, at any time, in `/compaction-router` and in `compaction-router/ledger.jsonl`.
